...
def activate_account(request, token):
    ...
    if User.objects.filter(email=email).exists():
        request.session["verify_success"] = "Your account is already active. Please sign in."
        return redirect("login-page")
    ...
    request.session["verify_success"] = (
        "Welcome! Your account has been verified. You can now sign in."
    )
    return redirect("login-page")
...
def _products_qs(search_query=""):
    # FIX ①: prefetch variation_images to avoid N+1 on the variation-count badge
    qs = Product.objects.prefetch_related("variation_images").filter(is_active=True).order_by("name")
    if search_query:
        qs = (
...
def _handle_update_send_request(request):
    ...
    VALID_STATUSES = {"pending", "quoted", "confirmed", "in_work", "shipped", "complete", "cancelled"}
    ...
    messages.success(
        request,
        f'Request #{req.pk} ({req.full_name}) updated to "{req.get_status_display()}".'
    )
    return redirect(f"{request.path}?tab=send_requests")
...
def admin_dashboard_data(request):
    ...
    if request.method == "GET":
        ...
        edit_product = (
            Product.objects
            .prefetch_related("variation_images")
            .filter(pk=edit_product_id)
            .first()
        ) if edit_product_id else None
    ...
    if action in ("add_product", "edit_product"):
        ...
        if action == "edit_product" and product_id:
            ...
            # FIX ③a: delete variation images that were checked for removal
            delete_ids = request.POST.getlist("delete_variation")
            if delete_ids:
                for vi in ProductImage.objects.filter(product=p, id__in=delete_ids):
                    try:
                        vi.image.delete(save=False)
                    except Exception as e:
                        logger.warning("Could not delete variation file: %s", e)
                    vi.delete()

            # FIX ③b: save / replace variation images for slots 1–5
            for position in range(1, 6):
                img_file = request.FILES.get(f"variation_image_{position}")
                label    = request.POST.get(f"variation_label_{position}", "").strip()
                if img_file:
                    existing = ProductImage.objects.filter(product=p, position=position).first()
                    if existing:
                        try:
                            existing.image.delete(save=False)
                        except Exception as e:
                            logger.warning("Could not delete old variation at slot %s: %s", position, e)
                        existing.image = img_file
                        existing.label = label
                        existing.save()
                    else:
                        ProductImage.objects.create(
                            product=p, image=img_file, label=label, position=position
                        )
                elif label:
                    # No new file but label was edited — update label only
                    ProductImage.objects.filter(product=p, position=position).update(label=label)
        ...
        # ── ADD new product ───────────────────────────────────────────────
            ...
            try:
                ...
                # FIX ④: save variation images for the new product
                for position in range(1, 6):
                    img_file = request.FILES.get(f"variation_image_{position}")
                    label    = request.POST.get(f"variation_label_{position}", "").strip()
                    if img_file:
    ...
    if action == "delete_product":
        ...
        # FIX ⑤: remove all variation image files from Firebase before deleting the product
        for vi in p.variation_images.all():
            try:
                vi.image.delete(save=False)
            except Exception as e:
                logger.warning("Could not delete variation image file: %s", e)
            vi.delete()
